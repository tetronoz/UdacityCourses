...
def validate_date(line):
    ...
    if match:
        if (int(match.group()) > 1885) and (int(match.group()) < 2015):
            return int(match.group())    
    return False
    
def validate_uri(line):
    match = re.match(r'http://dbpedia.org*', line)
    if match:
        return True
    else:
        return False
    ...
    with open(input_file, "r") as f:
        ...
        for row in reader:
            if (validate_uri(row['URI'])):            
                nd = validate_date(row['productionStartYear'])
                if (nd):
                    row['productionStartYear'] = nd
                    GOODDATA.append(row)
                else:
                    BADDATA.append(row)                                
