...
def article_overview(kind, period):
    ...
    for i in range(0, l):
        ...
        medialen = len(data[i]["media"])
        for z in range(0, medialen):
            mlen = len(data[i]["media"][z]["media-metadata"])
            for y in range(0, mlen):
                if data[i]["media"][z]["media-metadata"][y]["format"] == "Standard Thumbnail":
                    urls.append(data[i]["media"][z]["media-metadata"][y]["url"])
